...
Two accuracy sources are used — each labelled clearly:

- **†** = airline tweet sample (same 1,000 rows, cross-domain transfer — models trained on Amazon/IMDB/financial, *not* Twitter)
- **★** = full held-out benchmark (n = 1,247 real-only, Amazon/IMDB/tweets/financial mix; gold standard)
...
| **sentiment.ai e5-small** | **310** | neural, CPU |
| **sentiment.ai e5-base** | **141** | neural, CPU, 768-d |
| sentiment.ai openai | network-dependent | API, ~300–600 t/s typical |

Lexicon methods win on throughput (dictionary lookups, not neural inference). `sentiment.ai`
CPU speed is bounded by the embedding step; a GPU gives ~10–50× on the same hardware.

## R — accuracy

| method | macro-F1 (†) | macro-F1 (★) | notes |
|:-------|:-----------:|:------------:|:------|
| **sentiment.ai e5-base** | 0.595 | **0.899** | best on-device; ties OpenAI on ★ |
| **sentiment.ai openai** | — | **0.886** | API, text leaves your machine |
| **sentiment.ai e5-small** | 0.613 | **0.854** | default; matches old TF default on ★ |
| tidytext (bing) | 0.511 | — | binary lexicon forced to 3-class |
| syuzhet (AFINN) | 0.301 | — | word-score sum |
| sentimentr | 0.293 | — | sentence-aware lexicon |

On the held-out benchmark (★), `e5-base` ties paid OpenAI and beats `e5-small` by ~5 pp.
The airline-tweet sample (†) shows `e5-base` slightly *below* `e5-small` — within sampling
variance on a domain neither model was trained on; the gold-standard ★ numbers are the
reliable read.

## Python — speed

Measured on 1,000 airline tweets, CPU-only (median of 3 runs):
...
because it avoids the R↔Python round-trip. Both benefit equally from a GPU.

## Python — accuracy

| method | macro-F1 (†) | notes |
|:-------|:-----------:|:------|
| HF twitter-roberta | 0.777 | ⚠️ near-in-domain — see caveat below |
...
⚠️ **twitter-roberta is near-in-domain here.** It was fine-tuned on TweetEval (Twitter
data overlapping with airline tweets). Its F1=0.777 is not a fair out-of-sample comparison.

**The harder, fairer test:** on a mixed-domain corpus (Amazon/IMDB/financial/verbatims),
`sentimentai-py e5-base` reaches macro-F1 **0.899**, matching paid OpenAI (0.886).
twitter-roberta would be expected to *under*perform there — it was never trained on
financial news or long-form reviews. Domain-specialised models look best on their own
domain; cross-domain transfer is where `sentiment.ai`'s generalisation shines.

**When to use each:**

| | sentimentai e5-small | sentimentai e5-base | twitter-roberta | vaderSentiment |
|:--|:--:|:--:|:--:|:--:|
| Cross-domain accuracy | ✓ | **✓✓** | ✗ (domain-locked) | ✗ |
| Multilingual (~100 lang) | **✓** | **✓** | ✗ (English only) | ✗ |
