...
| Macro-F1 (real text) | 0.899 |

**Best for:** final datasets for publication, longer/more nuanced text (interview transcripts,
open-ended verbatims), servers with ample RAM, cases where the ~5 pp F1 gain matters.
...
| Macro-F1 (real text) | 0.886 |
| Cost | per-token API charge |

Text leaves your machine and is sent to OpenAI's servers. Requires an API key.
Accuracy ties `e5-base` on most corpora but has ongoing cost and a network dependency.
...
All benchmarks run on **1,000 airline tweets** (random sample; macOS aarch64 M-series,
8 CPU cores). Accuracy = macro-F1 on 3-class negative/neutral/positive against crowd-sourced
labels. Speed = median of 3 timed runs.

**Important caveat:** the airline tweet corpus is a challenging domain (high proportion
of negative tweets, short informal text). Macro-F1 figures here are lower than the
out-of-sample figures in the full benchmark (which includes Amazon/IMDB/financial corpora).
The relative ordering within each language is the reliable signal.

## R — speed

| method | texts/sec | notes |
|:-------|----------:|:------|
| tidytext (bing) | **32,258** | pure R, in-memory dictionary lookup |
| syuzhet (AFINN) | 3,311 | pure R, word-score sum |
| sentimentr | 564 | R + C++, sentence-aware |
| **sentiment.ai e5-small** | **310** | Python backend, 8-core parallel embedding |
| **sentiment.ai e5-base** | **141** | Python backend, 768-d embeddings |

Lexicon methods are faster because they do dictionary lookups, not neural inference.
The `sentiment.ai` speed on CPU is limited by the embedding step; a GPU or batch-optimised
environment would give 5–20× improvement on the same hardware.

## R — accuracy

| method | macro-F1 | % agreement | notes |
|:-------|:--------:|:-----------:|:------|
| **sentiment.ai e5-small** | **0.613** | **73.6%** | 3-class, calibrated |
| **sentiment.ai e5-base** | **0.595** | **73.7%** | 3-class, calibrated |
| tidytext (bing) | 0.511 | 51.7% | binary lexicon, mapped to 3-class |
| syuzhet (AFINN) | 0.301 | 28.3% | word-score sum, airline domain mismatch |
| sentimentr | 0.293 | 28.4% | Jockers-Rinker lexicon, airline domain mismatch |

`sentiment.ai` leads on accuracy despite the airline-tweet domain mismatch with its
training data. Lexicon methods (AFINN, sentimentr) suffer because their word lists
don't cover airline-specific vocabulary and slang well.

For `e5-base` scoring slightly lower than `e5-small` on this specific sample: this is
within sampling variance (different 1000-row draw) — the full-corpus benchmark
(n = 1,247 real-only) shows e5-base at 0.899 vs e5-small at 0.854.

Both `sentiment.ai` models were trained on Amazon reviews, IMDB, and financial news — *not*
on Twitter data. The F1 figures above reflect cross-domain transfer, which is the practically
relevant test for most users.

## Python — speed

Measured on 1,000 airline tweets, macOS aarch64, CPU-only (median of 3 runs):

| method | texts/sec | notes |
|:-------|----------:|:------|
| vaderSentiment | **4,875** | pure Python, rule-based lexicon |
| TextBlob | 4,182 | NaiveBayes + pattern rules |
| **sentimentai-py (e5-small)** | **485** | neural, sentence-transformers backend |
| HF twitter-roberta (fine-tuned) | 19 | fine-tuned transformer, CPU bottleneck |

`sentimentai-py` is faster than the equivalent R package call (~485 vs ~310 t/s) because
it avoids the R-to-Python round-trip overhead. A GPU would give 10–50× for both the
sentimentai and roberta paths.

## Python — accuracy

Macro-F1 on the same 1,000 airline-tweet sample (3-class neg/neu/pos against crowd labels):

| method | macro-F1 | % agreement | notes |
|:-------|:--------:|:-----------:|:------|
| HF twitter-roberta (fine-tuned) | 0.777 | 77.0% | ⚠️ trained on Twitter data — see note |
| **sentimentai-py (e5-small)** | **0.615** | **71.4%** | trained on Amazon/IMDB/financial mix |
| vaderSentiment | 0.473 | 55.9% | weak on neutral; airline vocab mismatch |
| TextBlob | 0.349 | 43.3% | general-purpose, not tuned |

⚠️ **Important caveat on twitter-roberta:** `cardiffnlp/twitter-roberta-base-sentiment-latest`
is fine-tuned on the TweetEval corpus, which overlaps substantially with the airline-tweet-style
Twitter data used here. Its F1=0.777 reflects in-domain or near-in-domain performance, not
a fair out-of-sample comparison. Treat it as "upper bound when you fine-tune on your target
domain" rather than a general-purpose accuracy number.

`sentimentai-py`'s F1=0.615 on airline tweets is arguably *more impressive* in context: it
was trained on a general mix (Amazon reviews, IMDB, financial news, short verbatims) and
transfers to a Twitter domain with no adaptation.

**When to use each:**

- **twitter-roberta**: if you need maximum accuracy on English Twitter and are comfortable with a ~500 MB model that is slow on CPU, not multilingual, and not tunable. Check for overlap with your test domain before trusting the numbers.
- **sentimentai-py**: generalises across languages and domains; tunable poles; calibrated confidence; auditable provenance; matches the R package bit-for-bit.
- **vaderSentiment / TextBlob**: when you need sub-millisecond speed and accuracy is a distant second priority.
