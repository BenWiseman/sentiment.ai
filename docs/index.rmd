...
1) It is **case-Insensitive**, can **handle spelling mitsakes**, and can be applied to **dieciséis (16) languages**! 
...
3) You can **choose the context** for what negative and positive mean using the `sentiment_match()` function. For example, you could set `positive` to mean `"high quality"` and negative to mean `"low quality"` when looking at product reviews.
...
You'll probably want to initialize the language embedding model first if you want to 1) not use the default models, and 2) want to re-apply sentiment scoring without the overhead of preparing the embedding model in memory. `init_sentiment.ai()` will create a function called `sentiment.ai_embed` in your global environment. This is necessary for downstream sentiment scoring and matching. As an added bonus you can call this manually to turn text into a numeric embedding vector (e.g if you want to cluster comments).

`init_sentiment.ai()` has the following paramaters:
...
* "The RETICULATE_PYTHON environment variable is set, which can be due to",
